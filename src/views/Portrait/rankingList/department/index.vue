...
@Component({
  ...
  async mounted (){
    //默认展示当月数据
    this.searchFilter.startTime = dayjs(new Date()).startOf('month').format('YYYY-MM-DD HH:mm:ss');
    //this.searchFilter.startTime = '2021-07-01 00:00:00';
    ...
    console.log("测试！");
    console.log(this.allProIndiactor);
   
  },
  methods: {
    //about skillMapranking
    skillMapranking(){
      console.log(this.searchFilter.startTime);
      return new Promise(resolve => {
        RankingServer.deptSkillMap(this.searchFilter).then(res => {
    ...
    async projectindiator(){
      const res = await RankingServer.deptPro(this.searchFilter);
